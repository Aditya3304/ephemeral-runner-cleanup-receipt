...
cp /etc/ssl/certs/ca-certificates.crt .build/finalizer-image/ca-certificates.crt
...
tag="cleanup-receipt/finalizer:$(sha256sum .build/finalizer-image/{finalizer,proofctl,archivectl,cosign,Dockerfile,ca-certificates.crt} | sha256sum | cut -c1-32)"
