...
async function validateAssignment(c, provider, env = process.env) {
  ...
  const assignment = JSON.parse(bytes.toString("utf8"));
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (value === void 0 || value === "" || id[field] !== value) throw new Error(`Assignment GitHub identity mismatch: ${field}`);
  }
}
