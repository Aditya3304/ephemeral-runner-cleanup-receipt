...
export type Stage = (config: Config, data: Record<string, string>) => Promise<void>;
export function checkPayload(data: Record<string, string>): void {
...
export const stageKubernetes: Stage = async (c, data) => {
  checkPayload(data);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const finish = (error?: Error) => {
      ...
      if (error) { req.destroy(); reject(error); } else resolve();
    };
    const req = request({
      hostname: c.apiHost.replace(/^\[|\]$/g, ''), port: c.apiPort,
      path: `/api/v1/namespaces/${encodeURIComponent(c.stageNamespace)}/configmaps/${encodeURIComponent(c.stageName)}`,
      method: 'PATCH', ca, rejectUnauthorized: true, agent: false,
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/merge-patch+json', 'Content-Length': Buffer.byteLength(body) },
    }, res => {
      // Reject redirects, authentication failures and missing maps; never create a fallback map.
      if (res.statusCode !== 200) { res.destroy(); finish(new Error(`Staging PATCH rejected: HTTP ${res.statusCode ?? 'unknown'}`)); return; }
      let bytes = 0;
      res.on('data', (chunk: Buffer) => { bytes += chunk.length; if (bytes > 2 * 1024 * 1024) finish(new Error('Staging response too large')); });
      res.on('end', () => finish());
